...
-- 3. Создаем функцию получения активных навыков
...
-- 4. Создаем функцию получения пассивных навыков
...
-- 5. Проверяем что функции созданы правильно
SELECT 'ПРОВЕРКА ФУНКЦИЙ:' as info;
SELECT 
    proname as function_name,
    pg_get_function_arguments(oid) as arguments
FROM pg_proc 
WHERE proname IN ('learn_active_skill', 'get_character_active_skills', 'get_character_passive_skills')
AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');

SELECT '✅ КОНФЛИКТ ФУНКЦИЙ ИСПРАВЛЕН!' as result;
